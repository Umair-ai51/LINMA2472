...
for step in 1:num_steps
    ...
    # Positional encoding (should be fixed, not random each time ideally)
    P = trnsf_Node(randn(d_model, block_size))
    ...
    # Update W_embed only (as an example)
    grad = get_gradient(W_embed)
    
    # Gxradient descent update
    for i in 1:vocab_size
        W_embed.value[:, i] .-= η * grad[:, i]
    end
    
    # Reset gradients
    W_embed.derivative .= 0.0
